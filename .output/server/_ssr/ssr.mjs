...
var ENQUIRIES_PATH = fileURLToPath(new URL("../data/enquiries.json", import.meta.url));
...
async function handleEnquiryRequest(request) {
	...
	try {
		...
		const payload = Object.fromEntries(Object.entries(body).filter(([key, value]) => allowedKeys.has(key) && value !== void 0));
		...
		if (!name || !email || !phone || !course || !mode) return new Response(JSON.stringify({
			success: false,
		...
		if (!content.trim()) existing = [];
			...
			if (!Array.isArray(parsed)) throw new Error("Stored enquiries data is invalid.");
			existing = parsed;
		}
		const enquiry = {
		...
		const nextEntries = [...existing, enquiry];
...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-PBlokqOB.mjs").then((n) => n.t).then((m) => m.default ?? m);
...
var server_default = { async fetch(request, env, ctx) {
	if (new URL(request.url).pathname === "/api/enquiries") return handleEnquiryRequest(request);
	try {
